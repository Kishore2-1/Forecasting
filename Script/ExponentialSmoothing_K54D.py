...
file_path = 'K54Ddata_34812636.xlsx'
...
plt.plot(data['Date'], data['K54D'], marker='o', color='black')
...
plt.legend()
plt.subplot(4, 1, 4)
plt.plot(data.index, data['K54D'], color='black', label='Original')
plt.legend()
plt.tight_layout()
plt.show()
...
# Plot
plt.figure(figsize=(10, 6))
plt.scatter(data['Year'], data['K54D'], color='black', alpha=0.5) 
plt.title('Scatter Plot for Average Weekly Earning')
plt.xlabel('Year')
plt.ylabel('Average Weekly Earning')
plt.grid(True)
plt.tight_layout()
...
# Plot ACF
plt.figure(figsize=(10, 6))
plot_acf(data['K54D'], lags=50, ax=plt.gca(), color='black')
plt.title('Autocorrelation Function for Average Weekly Earning')
plt.xlabel('Lag')
